...
for target in debug tests release; do
  ...
  "${AARCH64_TOOLCHAIN_PREFIX}ld" --fix-cortex-a53-835769 --fix-cortex-a53-843419 --no-warn-rwx-segments -N -Ttext=0x0 -o "${target}.elf" "${target}.o"
  "${AARCH64_TOOLCHAIN_PREFIX}objcopy" --set-start=0x0 "${target}.elf" -O binary "${target}.img"
